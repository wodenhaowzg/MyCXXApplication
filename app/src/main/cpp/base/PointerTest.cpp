...
void createPointer() {
    ...
    // 定义一个指针，并将变量赋值给指针。
    int *pointer = &a;
    LOGI(BASETAG, "将变量 a 赋值给指针 pointer ，pointer 的值 = %p ，值里面的内容 = %d ，pointer 自身的地址 = %p", pointer, *pointer, &pointer);
    // 再次改变指针的值
    pointer = &b;
    LOGI(BASETAG, "将变量 b 赋值给指针 pointer ，pointer 的值 = %p ，值里面的内容 = %d ，pointer 自身的地址 = %p", pointer, *pointer, &pointer);
}
